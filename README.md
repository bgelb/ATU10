...
Baud rate is limited to 1200 8N1 because RD2 has a 100 nF capacitor and 1 K series resistor that low-pass filters the incoming serial line. Higher baud rates are unreliable unless you remove C8, C9, C10, and C11 (the EXT jack RC network). When `EXT_SERIAL_DEBUG` is enabled, the EXT jack external-control behavior is disabled to avoid pin conflicts.
